...
for group in range(groups):
        plt.scatter([y for (x,y, z) in group_record if test(z, group) and y > lower_bound], [math.log(10000 / x * 1000) for (x,y, z) in group_record if test(z, group) and y > lower_bound], marker='x', label=f'FLINNG, {names[index - 1]} in group {group}' )
...
plt.scatter([y for (x,y, z) in normal_record if x != 0  and y > lower_bound], [math.log(10000 / x * 1000) for (x,y, z) in normal_record if x != 0  and y > lower_bound], marker='s', label='FLASH')
